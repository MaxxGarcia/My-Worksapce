...
class BreakOut extends Component {
  ...
  setUpContext() {
    ...
    const draw = () => {
      const noRouteChange = this.props.location.pathname === "/BreakOut";
      noRouteChange && ctx.clearRect(0, 0, this.canvas.current.width, this.canvas.current.height)
      noRouteChange && drawBricks();
      noRouteChange && drawBall();
      noRouteChange && drawPaddle();
      noRouteChange && drawScore();
      noRouteChange && drawLives();
      noRouteChange && collisionDetection();
      if (y + dy < ballRadius) {
        dy = -dy;
...
export default withRouter(BreakOut);
